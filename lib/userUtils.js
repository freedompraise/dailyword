...
async function getUserByChatId(chatId, createIfMissing = true) {
  try {
    const { data, error } = await repo.getUserByChatId(chatId);
...
async function ensureUser(chatId) {
  try {
    ...
    // Try to get existing user first
      const { data: existing, error: fetchError } = await repo.getUserByChatId(chatId);
    ...
    // Create new user
    const { data: newUserData, error: insertErr } = await repo.insertUser({
    ...
    const { error: statsErr } = await repo.insertUserStats({
...
async function getUserById(userId) {
  try {
    const { data, error } = await repo.getUserById(userId);
