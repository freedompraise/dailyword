// lib/utils.js - Utility functions for bot interactions
...
function hasReceivedTodayWords(userWords, pendingWords = []) {
  const hasPending = Array.isArray(pendingWords) && pendingWords.some(pw => {
    const offeredAt = pw.offered_at ? new Date(pw.offered_at) : null;
    const today = new Date();
    today.setUTCHours(0, 0, 0, 0);
    return offeredAt && offeredAt.getTime() >= today.getTime();
  });

  if (!userWords || userWords.length === 0) return hasPending;
  const today = new Date();
  today.setUTCHours(0, 0, 0, 0);
  const todayISO = today.toISOString();
  
  const hasServed = userWords.some(uw => {
    const servedAt = new Date(uw.served_at);
    return servedAt.getTime() >= today.getTime();
  });

  return hasServed || hasPending;
}
...
function getHelpMessage() {
  ...
  message += `/review - Practice words due for review\n`;
  message += `/progress - Check your learning progress and streak\n`;
  message += `/contact - Send a message to the admin\n`;
