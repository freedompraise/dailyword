...
/**
 * Create keyboard for daily word card (new words - no practice option)
 * @param {number} wordId - Word ID
 * @returns {Object} Reply markup
 */
function createNewWordCardKeyboard(wordId) {
  return createInlineKeyboard([
    [
      createButton('📖 Show definition', `word:show:${wordId}:np`)
    ]
  ]);
}

/**
 * Create keyboard for word card (used in review/today views - includes practice)
 * @param {number} wordId - Word ID
 * @returns {Object} Reply markup
 */
function createWordCardKeyboard(wordId) {
  return createInlineKeyboard([
    [
      createButton('📖 Show definition', `word:show:${wordId}:p`),
 ...
 * @returns {Object} Reply markup
 */
function createDefinitionKeyboard(wordId, hasSecondExample = false, allowPractice = true) {
  const buttons = [];

  // Only show "See another example" if there's a second example
  if (hasSecondExample) {
    buttons.push([
      createButton('📝 See another example', `word:example:${wordId}:${allowPractice ? 'p' : 'np'}`)
    ]);
  }

  // Always show back button
  buttons.push([
    createButton('◀️ Back to word', `word:back:${wordId}:${allowPractice ? 'p' : 'np'}`)
  ]);
...
function createFeedbackKeyboard(wasCorrect, sessionId = null, nextWordId = null) {
  if (sessionId && nextWordId) {
    return createInlineKeyboard([
      [
        createButton('➡️ Next word', `session:${sessionId}:next:${nextWordId}`)
      ],
      [
